...
// Calculate rewards (1 per 10 orders OR 1 per ₹1000 spent)
function calculateRewards(customer) {
  const rewardFromOrders = Math.floor(customer.totalOrders / 10);
  const rewardFromAmount = Math.floor(customer.amount / 1000);
  return Math.max(rewardFromOrders, rewardFromAmount);
}
...
function renderCustomers(filtered = customers) {
  ...
  customersList.innerHTML = filtered.map((c, i) => {
    ...
    const remainingRewards = totalRewards - (c.redeemed || 0);
    const rewardIcon = remainingRewards > 0 ? "🎉" : "";
    const redeemStatus = remainingRewards > 0 ? remainingRewards : "No Rewards";
    ...
    return `
      <tr>
        ...
        <td class="p-2">${rewardIcon}</td>
        <td class="p-2">${c.amount ? c.amount.toFixed(2) : "0.00"}</td>
        <td class="p-2">
          <button data-index="${i}" class="redeem-btn bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">${redeemStatus}</button>
        </td>
...
💰 Amount Spent: ₹${c.amount.toFixed(2)}
🎁 Total Rewards Earned: ${totalRewards}
🎯 Remaining Rewards: ${remainingRewards}

✨ After 10 Orders you earn 1 Reward Point = 1 Free Meal
💎 Or spend ₹1000 = 1 Free Meal
